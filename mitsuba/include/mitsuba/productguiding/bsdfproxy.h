...
class BSDFProxy
{
  public:
    ...
    inline float ggx_lobe_incoming(const float cos_lobe_in, const float alpha) const
    {
        const float cos2_lobe_in = cos_lobe_in * cos_lobe_in;
        const float sin2_lobe_in = 1.0f - cos2_lobe_in;
        const float alpha2 = alpha * alpha;
        ...
        factor *= factor;
        factor *= M_PI * alpha2;
        return factor == 0.0f ? 0.0f : 1.0f / factor;
    }
    ...
    inline float evaluate_diffuse(
        const Vector3f &incoming, const Vector3f &normal) const
    {
        return std::max(dot(normal, incoming), 0.0f);
    }

    inline float evaluate_reflectance(
        const Vector3f &incoming, const Vector3f& reflection_lobe, const float alpha) const
    {
        const float cos_refl_i = dot(reflection_lobe, incoming);

        if (cos_refl_i > 0.0f)
        {
            return ggx_lobe_incoming(cos_refl_i, alpha * 2.0f); // roughness correction
        }
